...
// 1. INVENTORY (Cards)
// ==========================================
async function loadInventory(userRole) {
    const list = document.getElementById("medsList");
    if (!list) return;

    try {
        const meds = await window.medicationService.getAll();
        list.innerHTML = "";
...
// ==========================================
async function loadChecklist(dateStr) {
    const tableBody = document.getElementById("checklistBody");
    tableBody.innerHTML = "<tr><td colspan='6' class='text-center p-4 text-muted'>Loading logs...</td></tr>";

    try {
        const [meds, logs] = await Promise.all([
            window.medicationService.getAll(),
            window.medicationService.getLogsByDate(dateStr)
        ]);
